...
class App extends Component {
  ...
  state = {
    ...
    error: false,
    errorMessage: "",
    loggedInUserId:1,     // TODO: pull member id of the logged in user from database
    isPaidMember:true,   // TODO: pull membership status from database
  ...
  componentDidMount() {
    ...
    console.log("loggedInuser: "+this.state.loggedInUserId)
  ...
  */
  ...
  render() {
    ...
    async function handleToken(token, addresses){
        ...
        const response=await axios.post('http://127.0.0.1:8002/checkout',{token});
        console.log(response)
        const {status}=response.data
        if(status === 'success'){
          that.setState({ isPaidMember: true });
        }else{
            that.setState({isPaymentError:true})
        }

    }
    return (
      <div>
        {this.state.isPaidMember ? (
        <div>
        ...
        <Modal show={this.state.signInModalShow}>
          ...
          <Modal.Body>
            <UserList
              userData={this.state.users.filter(u => u.id === this.state.loggedInUserId)}
        ...
        <Modal show={!(this.state.isPaidMember)}>
            <Modal.Header>
              <Modal.Title>Access Messaging:</Modal.Title>
            </Modal.Header>

            <Modal.Body>
              <h1 style={{color: "gray", alignItems:"center"}}>Want to access messaging? Consider joining for $5/month</h1>
              <div style={{margin: "25px 0px 25px 0px"}}>
                  <div style={{display: "block", margin: "0 auto" }}>
                      <StripeCheckout
                      stripeKey="pk_test_Obq0r6YqPDV7QBmv83UkWe4A00Q2GFgfQo" //TODO: REPLACE THIS KEY WITH THE LIVE PUBLISHABLE KEY 
                      token={handleToken}
                      billingAddress
                      amount={5 * 100}
                  ...
                  <div>
                     {
                      this.state.isPaymentError ? (<div style={{background:"red",textColor:"white"}}> ERROR OCCURED !!</div>):
                      (<div></div>)
                     }  
                  </div>
              </div>
